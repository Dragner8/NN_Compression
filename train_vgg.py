...
if __name__ == '__main__':
    ...
    net, cfg , masks,codebooks = load_compress_model("quant", cifar=args.cifar)
    print(cfg)
    ...
    for epoch in range(1,14):
        ...
        train(epoch)
        cmodel = compress_vgg19_bn(cnfg= cfg,num_class=args.cifar)
        cmodel.load_state_dict(net.state_dict())
        cmodel, masks = VGGpruner.prune_weights(cmodel, masks=masks)
        cmodel, codebooks = quantizer.kmeans_quant(cmodel,codebooks=codebooks)
        #net,masks=VGGpruner.prune_weights(net,masks=masks)
        ...
        if epoch > -1 and best_acc < acc:
            ...
            save_compress_model(net, checkpoint_path.format(net=NAME, epoch=epoch, type='best'), cfg=cfg,mask=masks)
            best_acc = acc
            continue

        if not epoch % 6:
            save_compress_model(net,checkpoint_path.format(net=NAME, epoch=epoch, type='regular'),cfg=cfg,mask=masks)
